class FavoritesController < ApplicationController
  ...
  def current_user_must_be_favorite_scout
    favorite = Favorite.find(params[:id])

    unless current_user == favorite.scout
  ...
  def index
    ...
    @favorites = @q.result(:distinct => true).includes(:scout, :player).page(params[:page]).per(10)
  ...
  def create
    ...
    @favorite.scout_id = params[:scout_id]
    @favorite.player_id = params[:player_id]
